...
const cfg = JSON.parse(fs.readFileSync(path.join(__dirname, 'data.json'), 'utf8'))

const JPNAME = 0, RONAME = 1, ENNAME = 2
const DAISANGEN = 37, DAISUUSHI = 50, TSUMOGIRI = 60
export const TENHOU_PARSER_VERSION = 2
...
function splitCalledTiles(tileTexts, calledTile, fallbackIndex, handCount) {
  const tiles = tileTexts.map(t => Tile.parse(t))
  const called = calledTile || tiles[fallbackIndex] || tiles[tiles.length - 1]
  let calledIndex = tiles.findIndex(t => t.equals(called))

  if (calledIndex === -1) {
    calledIndex = tiles.findIndex(t => t.deaka().equals(called.deaka()))
  }
  if (calledIndex === -1) calledIndex = fallbackIndex

  const hands = tiles.filter((_, index) => index !== calledIndex)
  return {
    called,
...
class DaiminkanSymbol {
  ...
  encodeTenhou() {
    ...
    let t = [this.a.encodeTenhou(), this.b.encodeTenhou(), this.c.encodeTenhou()]
...
class Kyoku {
  constructor(nplayers, round, initscores, doras, draws, discards, haipais, poppedtile, dealerseat) {
    ...
    this.dealerseat = dealerseat; this.ldseat = -1; this.ldtile = null; this.nriichi = 0; this.priichi = false; this.nkan = 0;
  ...
  dump() {
    ...
    // 始终按 4 玩家布局展开（第 4 家在三麻时留空），
    // 使 result 固定在 index 16，与四麻一致：homura 读 [13] 为第 4 家手牌（合法 tile 数组）蒙混通过。
    // 三麻第 4 家无手牌，补 13 个 0 占位，保证 [13] 为长度 13 的合法数组。
    for (let i = 0; i < 4; i++) {
      let hp = (this.haipais[i] || []).map(t => t.encodeTenhou())
      if (i === 3 && hp.length === 0) hp = Array(13).fill(0)
      const dr = (this.draws[i] || []).map(t => t.encodeTenhou())
      const dc = (this.discards[i] || []).map(t => t.encodeTenhou())
      entry.push(hp)
      entry.push(dr)
      entry.push(dc)
    }
...
export class MajsoulPaipuParser {
  ...
  handleGameRecord(record) {
    let res = {
      ver: "2.3", ref: record.head.uuid, ratingc: "", rule: {}, lobby: 0, dan: [], rate: [], sx: [], name: [], sc: [], title: [], log: []
    }
    res._parser_version = TENHOU_PARSER_VERSION
  ...
  _handleChiPengGang(log) {
    ...
    let seat = log.seat
    if (log.type === 0) {
      const { called, hands } = splitCalledTiles(log.tiles, this.kyoku.ldtile, 1, 2)
      this.kyoku.draws[seat].push(new ChiSymbol(hands[0], hands[1], called))
    } else if (log.type === 1) {
      const { called, hands } = splitCalledTiles(log.tiles, this.kyoku.ldtile, 2, 2)
      let idx = relativeSeating(seat, this.kyoku.ldseat)
      this.kyoku.countpao(called, seat, this.kyoku.ldseat)
      this.kyoku.draws[seat].push(new PonSymbol(hands[0], hands[1], called, idx))
    } else if (log.type === 2) {
      const { called, hands } = splitCalledTiles(log.tiles, this.kyoku.ldtile, 3, 3)
      let idx = relativeSeating(seat, this.kyoku.ldseat)
      this.kyoku.countpao(called, seat, this.kyoku.ldseat)
      this.kyoku.draws[seat].push(new DaiminkanSymbol(hands[0], hands[1], hands[2], called, idx))
  ...
  _handleHuLe(log) {
    ...
    const currentRound = this.kyoku.round
    this.kyoku.result = {
      uras: ura,
      dump: () => {
        let li = [RUNES.agari[JPNAME]]
        for (let a of agari) {
          li.push(padList(a.delta.map(x => Math.round(x / 1000)), 4, 0))
          ...
          for (let e of a.yaku) {
            let name = e.name(currentRound, a.seat)
